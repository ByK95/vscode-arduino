...
export class DeviceContext implements IDeviceContext, vscode.Disposable {
  ...
  private constructor() {
    if (vscode.workspace && ArduinoWorkspace.rootPath) {
      this._watcher = vscode.workspace.createFileSystemWatcher(
          path.join(ArduinoWorkspace.rootPath, ARDUINO_CONFIG_FILE));
      ...
      this._vscodeWatcher = vscode.workspace.createFileSystemWatcher(
          path.join(ArduinoWorkspace.rootPath, ".vscode"), true, true, false);
      ...
      this._sketchStatusBar = vscode.window.createStatusBarItem(
          vscode.StatusBarAlignment.Right, constants.statusBarPriority.SKETCH);
      this._sketchStatusBar.command = "arduino.setSketchFile";
      this._sketchStatusBar.tooltip = "Sketch File";
    }
    const autoResolveSketch =
        VscodeSettings.getInstance().autoResolveSketch if (autoResolveSketch) {
  ...
  public get extensionPath(): string { return this._extensionPath; }

  public set extensionPath(value: string) { this._extensionPath = value; }

  /**
   * TODO: Current we use the Arduino default settings. For future release, this
   * dependency might be removed
        ...
        .then(
            (files) => {
              ...
              if (files && files.length > 0) {
                ...
                deviceConfigJson = util.tryParseJSON(
            ...
            (reason) => {
              ...
              // vscode.workspace.findFiles() for some reason now throws an
              // error ehn path does not exist
              // vscode.window.showErrorMessage(reason.toString());
              // Logger.notifyUserError("arduinoFileUnhandleError", new
              // Error(reason.toString()));

              // Workaround for change in API, populate required props for
              // arduino.json
              this._port = null;
              this._board = null;
              this._sketch = null;
  ...
  public saveContext() {
    ...
    if (util.fileExistsSync(deviceConfigFile)) {
      deviceConfigJson =
          util.tryParseJSON(fs.readFileSync(deviceConfigFile, "utf8"));
    }
    if (!deviceConfigJson) {
    ...
    fs.writeFileSync(deviceConfigFile,
                     JSON.stringify(deviceConfigJson, (key, value) => {
                       if (value === null) {
                         return undefined;
                       }
                       return value;
  ...
  public get sketch() { return this._sketch; }
  ...
  public get prebuild() { return this._prebuild ? this._prebuild.trim() : ""; }
  ...
  public get debugger_() { return this._debugger; }
  ...
  public get programmer() { return this._programmer; }
  ...
  public async initialize() {
    if (ArduinoWorkspace.rootPath &&
      ...
      vscode.window.showInformationMessage(
          "Arduino.json is already generated.");
      return;
    } else {
      if (!ArduinoWorkspace.rootPath) {
      ...
      if (this.sketch) {
        ...
        vscode.window.showInformationMessage(
            "No *.ino sketch file was found or selected, so skip initialize command.");
      }
    }
  }
        ...
        .then(async(fileUris) => {
          if (fileUris.length === 0) {
            let newSketchFileName = await vscode.window.showInputBox({
              value: "app.ino",
              prompt:
                  "No .ino file was found on workspace, initialize sketch first",
            ...
            if (newSketchFileName) {
              const snippets = fs.readFileSync(
                  path.join(this.extensionPath, "snippets", "sample.ino"));
              fs.writeFileSync(
                  path.join(ArduinoWorkspace.rootPath, newSketchFileName),
                  snippets);
              ...
              vscode.window.showTextDocument(textDocument,
                                             vscode.ViewColumn.One, true);
            } else {
              this._sketch = undefined;
            }
          } else if (fileUris.length === 1) {
            this.sketch =
                path.relative(ArduinoWorkspace.rootPath, fileUris[0].fsPath);
          } else if (fileUris.length > 1) {
            const chosen = await vscode.window.showQuickPick(
                <vscode.QuickPickItem[]>fileUris.map(
                    (fileUri): vscode.QuickPickItem => {
                      return <vscode.QuickPickItem>{
                        label: path.relative(ArduinoWorkspace.rootPath,
                                             fileUri.fsPath),
